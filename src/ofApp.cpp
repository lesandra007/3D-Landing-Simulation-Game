...
void ofApp::update() {
	...
	if (bReverse) {
		...
		float dampingFactor = 0.8f;  // To reduce bouncing
		glm::vec3 newVelocity = reverseVelocity * dampingFactor;

		// update lander's position
		glm::vec3 offset = avgCollisionNormal * 0.5f;  // Move slightly away from surface
		glm::vec3 newPos = currPos + reverseVelocity * dt + offset;
